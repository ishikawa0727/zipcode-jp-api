...
export class ZipCodeJp {
   ...
   * 町域の「（...）」の部分を削除する
   */
  static minimize (data: ZipCodeData){
    return [
      data.zip_code,
      data.prefecture,
      data.city,
   ...
   */
  static makeFixTownConditionsIndexedByZipCode = (rows: string[][]) => {
    const zipCodeDataList = this.rowsToZipCodeDataList(rows)
   ...
   * データをファイルに保存する
   */
  static save (segmentalized: Record<string, string[][]>) {
    ...
    Object.entries(segmentalized).forEach(([key, rows]) => {
      const zipCodeDataList = this.rowsToZipCodeDataList(rows)
      const minimized = zipCodeDataList.map(this.minimize)
